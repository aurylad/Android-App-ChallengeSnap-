...
$statement = mysqli_prepare($con, "SELECT image_path, id_auto, image_name, date, challenge_title FROM images WHERE id_username = ? AND id_auto = (SELECT MIN(id_auto) + ? FROM images WHERE id_username = ?)");
mysqli_stmt_bind_param($statement, "sis", $username, $idAuto, $username);
